<template>
  <section class="company_solutions">
    <div class="container">
      <div class="block-title text-center">
        <p>company solutions</p>
        <h3>Independently Owned</h3>
        <div class="leaf">
          <img src="@/static/images/resources/leaf.png" alt="">
        </div>
      </div>
      <div class="row">
        <div class="col-xl-4 col-lg-4">
          <div class="company_solutions_single">
            <div class="company_solutions_image">
              <img src="@/static/images/resources/com_solutions_img-1.jpg" alt="">
            </div>
            <h2>Harvest Innovations</h2>
            <p>There are many variations of passages of available, but the majority have in some form,
              by humour words which don't look believable.</p>
          </div>
        </div>
        <div class="col-xl-4 col-lg-4">
          <div class="company_solutions_single">
            <div class="company_solutions_image">
              <img src="@/static/images/resources/com_solutions_img-2.jpg" alt="">
            </div>
            <h2>Agriculture farming</h2>
            <p>There are many variations of passages of available, but the majority have in some form,
              by humour words which don't look believable.</p>
          </div>
        </div>
        <div class="col-xl-4 col-lg-4">
          <div class="company_solutions_single">
            <div class="company_solutions_image">
              <img src="@/static/images/resources/com_solutions_img-3.jpg" alt="">
            </div>
            <h2>organic solutions</h2>
            <p>There are many variations of passages of available, but the majority have in some form,
              by humour words which don't look believable.</p>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<script>
    export default {
        name: "CompanySolutions"
    }
</script>

<style scoped>

</style>
